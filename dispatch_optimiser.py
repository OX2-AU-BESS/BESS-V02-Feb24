...
class dispatch_optimiser:
    ...
    def calculate_dispatch(self, timestamp,forecasts,actuals,bat_deg_df):
        ...
        if (self.revenue_method=='merchant'): 
            prob+= p.lpSum((((solar_dispatch[j]+battery_discharge[j])*self.gen.mlf_gen+(battery_charge[j])*self.gen.mlf_load)*self.optimisation_res/60*forecast_price_profile.iloc[j])
                           + solar_dispatch[j]*self.optimisation_res/60*self.scn.LGC_price
    ...
    """  
        ...
        for i in range(0,opt_len):
            ...
            prob += p.lpSum( ((battery_discharge[j]+(self.scn.FCAS_occurance*(raise6sec[j] + raise60sec[j] + raise5min[j] + raisereg[j])))*(1.0+eff)
                              +(battery_charge[j]+(self.scn.FCAS_occurance*(lower6sec[j] + lower60sec[j] + lower5min[j] + lowerreg[j])))*(1.0-eff))*self.optimisation_res/60 for j in range(0,i+1)) <= (self.gen.SOC-self.gen.min_SOC)*self.gen.bat_capacity     
        #print("boundary condition 1")
        # limit charge to maximum allowable SOC
        for i in range(0,opt_len):
            # total cumulated energy taken from Battery at any time must be less or equal to initial SOC* capacity - lower_limit
            prob += p.lpSum(-((battery_discharge[j]+(self.scn.FCAS_occurance*(raise6sec[j] + raise60sec[j] + raise5min[j] + raisereg[j])))*(1.0+eff)
                              +(battery_charge[j]+(self.scn.FCAS_occurance*(lower6sec[j] + lower60sec[j] + lower5min[j] + lowerreg[j])))*(1.0-eff))*self.optimisation_res/60 for j in range(0,i+1)) <= (self.gen.max_SOC-self.gen.SOC)*self.gen.bat_capacity     
        #print("boundary condition 1")
        ...
        prob += p.lpSum(((battery_discharge[j]+(self.scn.FCAS_occurance*(raise6sec[j] + raise60sec[j] + raise5min[j] + raisereg[j])))*(1.0+eff)
                         +(battery_charge[j]+(self.scn.FCAS_occurance*(lower6sec[j] + lower60sec[j] + lower5min[j] + lowerreg[j])))*(1.0-eff))*self.optimisation_res/60 for j in range(0,opt_len)) <= (-self.scn.target_SOC+self.gen.SOC + self.scn.SoC_tolerance)*self.gen.bat_capacity
        ...
        prob += p.lpSum( ((battery_discharge[j]+(self.scn.FCAS_occurance*(raise6sec[j] + raise60sec[j] + raise5min[j] + raisereg[j])))*(1.0+eff)
                          -(battery_charge[j]+(self.scn.FCAS_occurance*(lower6sec[j] + lower60sec[j] + lower5min[j] + lowerreg[j])))*(1.0-eff))*0.5*self.optimisation_res/60/self.gen.bat_capacity for j in range(0,opt_len) )<= ((self.scn.max_cycles)*self.optimisation_res/24)
        ...
        for i in range(0,opt_len):
            prob += p.lpSum(solar_dispatch[i]+battery_discharge[i]+battery_charge[i]+self.scn.FCAS_occurance*(raise6sec[i] + raise60sec[i] + raise5min[i] + raisereg[i])
                            +self.scn.FCAS_occurance*(lower6sec[i] + lower60sec[i] + lower5min[i] + lowerreg[i])) <= self.gen.plant_max_MW
